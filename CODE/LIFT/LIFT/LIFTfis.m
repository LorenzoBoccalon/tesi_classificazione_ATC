function [HammingLoss,RankingLoss,OneError,Coverage,Average_Precision,Outputs,Pre_Labels]=LIFTfis(train_data,train_target,test_data,test_target,ratio,svm)
...
for i=1:num_class
    ...
    USO(i)=0;
    if size(training_instance_matrix,2)>50
        USO(i)=1;
        y=training_label_vector;
        y(find(y==-1))=2;
        [out1{i}] =fsFisher(training_instance_matrix,y);
        training_instance_matrix=training_instance_matrix(:,out1{i}.fList(1:50));%nel paper originale (PRL) Nscelte era fissato a 2000
    end
...
for i=1:num_class
    ...
    if USO(i)==1
        testing_instance_matrix=testing_instance_matrix(:,out1{i}.fList(1:50));%nel paper originale (PRL) Nscelte era fissato a 2000
    end
